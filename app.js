...
function refreshInfografis() {
  syncOvJft();
  // Jika Firebase sudah siap, ambil data terbaru semua Bapas dulu sebelum render
  if (_fbReady && _fbDb) {
    var tanggal = getTanggalIso();
    loadAllBapasData(tanggal, function (all) {
      // Perbarui _parsedKlien dari data Firebase semua Bapas
      ...
      // Hanya update _parsedKlien jika ada data dari Firebase
      var totalFb = smrDew + smrAnak + bppDew + bppAnak + trkDew + trkAnak;
      if (totalFb > 0) {
      ...
      // Update input JFT dari data Firebase semua Bapas
    ...
    // Firebase belum siap, render dari data lokal yang ada
    syncInfTotalKlien();
    initCharts();
    renderJftDoughnuts();
  }
...
function syncDashboardFromBapasData(all) {
  ...
  // Jika halaman infografis sedang aktif, langsung perbarui chartnya
  var pageInfografis = document.getElementById("page-infografis");
  if (pageInfografis && pageInfografis.classList.contains("active")) {
    syncInfTotalKlien();
    initCharts();
    renderJftDoughnuts();
  }
